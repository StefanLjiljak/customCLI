...
function handleAPIError(err) {
    if (err.response.status === 401) {
        throw new Error(`${err.response.statusText} - Your API key is invalid - Go to https://nomics.com`)
    } else if (err.response.status === 404) {
        throw new Error(`${err.response.statusText} - Your API key is not responding`)
    } else {
